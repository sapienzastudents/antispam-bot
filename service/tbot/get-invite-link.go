...
func (bot *telegramBot) getInviteLink(chat *tb.Chat) (string, error) {
	...
	// The link was not found in the database/cache, so we generate a new link for the chat.
	//
	// Warning: "GetInviteLink" API will actually generate a new link instead of
	...
	if err != nil && err.Error() == tb.ErrGroupMigrated.Error() {
		...
		// Here we need to use Sprint() to convert migrate_to_chat_id because we
		// don't know if it's integer or string already.
