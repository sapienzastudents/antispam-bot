...
// spamFilter checks all text values in the slice against antispam functions. If a violation is detected, the
// corresponding action will be performed.
//
// Example: if the action on Chinese messages is delete, the bot will delete the message
//
// Time complexity: O(n * 2m) where "n" is the length of the textvalues slice, "m" is the length of the longest string
// in the slice
